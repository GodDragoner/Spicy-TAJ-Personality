...
if(!(isVar("HasExercised"))){
	...
	switch (getVar("ExerciseTimes")){
		...
		case 1:{
			 ...
			 setVar("GNMGold", getVar("GNMGold")+20 );
			 break;
		}
		case 0: {
	...
	if(getVar("ExerciseLevel")>getVar("ExerciseLevelOld")){
		sendVirtualAssistantMessage("Which is an improvement! %GNMEmoteHappy%"); 
	}
	if(getVar("ExerciseLevel") == getVar("ExerciseLevelOld")){
	 sendVirtualAssistantMessage("Which is a stay still :/ ");
	}
	if(getVar("ExerciseLevel")>getVar("ExerciseLevelOld")){
	 sendVirtualAssistantMessage("Which is sadly a decline! %GNMEmoteSad% ");
	}
