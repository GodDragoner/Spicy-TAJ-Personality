//(Reset)
//@NullResponse @SetVar[SpankingCount]=[0]
setVar("SpankingCount",0);

MXmessage(" %Fetch% %ChooseSpankingImplement% ", 12); 
MXlapmessage(random("Get over my lap and remember to count out loud..","Get over my lap","Get over my lap","Prepare yourself and get over my lap","Over my lap") + " %SlaveName%! ", 12);
//(Spanking)

while ( getVar("SpankingCount") < getVar("StrokesPerRound"))
{
spanks=randomInteger(1,5);

spanks=spanks*10;
switch(spanks)
{
	case 10:
	fastorslow=randomInteger(0,1);
	 setVar("SpankingCount",getVar("SpankingCount")+10 );
	 	MXoverlapmessage(random("left","left","bottom left","top left")+" " + random("ass!","asscheek!","buttcheek!","cheek!","rear cheek","rear","sitspots!") ); 
	 if (fastorslow==0)
		{ playAudio("Audio/Spicy/Punishment/SpankingTask/10Strikes/Spank10.mp3", true);
		 sleep(17);
		}
		else
		{ MXoverlapmessage(random("Fast ones!","Quick ones!","Speedy ones!"));
		 playAudio("Audio/Spicy/Punishment/SpankingTask/10Strikes/Spank10Fast.mp3", true);
		sleep(9);
		}
		
	fastorslow=randomInteger(0,1);
	 setVar("SpankingCount",getVar("SpankingCount")+10 );
	 	MXoverlapmessage(random("right","right","bottom right","top right")+" " + random("ass!","asscheek!","buttcheek!","cheek!","rear cheek","rear","sitspots!") ); 
	 if (fastorslow==0)
		{ playAudio("Audio/Spicy/Punishment/SpankingTask/10Strikes/Spank10.mp3", true);
		 sleep(17);
		}
		else
		{ MXoverlapmessage(random("Fast ones!","Quick ones!","Speedy ones!"));
		 playAudio("Audio/Spicy/Punishment/SpankingTask/10Strikes/Spank10Fast.mp3", true);
		sleep(9);
		}
		
	break;
	
	
		case 20:
	fastorslow=randomInteger(0,1);
	 setVar("SpankingCount",getVar("SpankingCount")+20 );
	 	MXoverlapmessage(random("left","left","bottom left","top left")+" " + random("ass!","asscheek!","buttcheek!","cheek!","rear cheek","rear","sitspots!") ); 
	 if (fastorslow==0)
		{ playAudio("Audio/Spicy/Punishment/SpankingTask/20Strikes/Spank20.mp3", true);
		 sleep(34);
		}
		else
		{ MXoverlapmessage(random("Fast ones!","Quick ones!","Speedy ones!"));
		 playAudio("Audio/Spicy/Punishment/SpankingTask/20Strikes/Spank20Fast.mp3", true);
		sleep(17);
		}
		
	fastorslow=randomInteger(0,1);
	 setVar("SpankingCount",getVar("SpankingCount")+20 );
	 	MXoverlapmessage(random("right","right","bottom right","top right")+" " + random("ass!","asscheek!","buttcheek!","cheek!","rear cheek","rear","sitspots!") ); 
	 if (fastorslow==0)
		{ playAudio("Audio/Spicy/Punishment/SpankingTask/20Strikes/Spank20.mp3", true);
		 sleep(34);
		}
		else
		{ MXoverlapmessage(random("Fast ones!","Quick ones!","Speedy ones!"));
		 playAudio("Audio/Spicy/Punishment/SpankingTask/20Strikes/Spank20Fast.mp3", true);
		sleep(17);
		}
		
	break;
	
	
		case 30:
	fastorslow=randomInteger(0,1);
	 setVar("SpankingCount",getVar("SpankingCount")+30 );
	 	MXoverlapmessage(random("left","left","bottom left","top left")+" " + random("ass!","asscheek!","buttcheek!","cheek!","rear cheek","rear","sitspots!") ); 
	 if (fastorslow==0)
		{ playAudio("Audio/Spicy/Punishment/SpankingTask/30Strikes/Spank30.mp3", true);
		 sleep(50);
		}
		else
		{ MXoverlapmessage(random("Fast ones!","Quick ones!","Speedy ones!"));
		 playAudio("Audio/Spicy/Punishment/SpankingTask/30Strikes/Spank30Fast.mp3", true);
		sleep(25);
		}
		
	fastorslow=randomInteger(0,1);
	 setVar("SpankingCount",getVar("SpankingCount")+30 );
	 	MXoverlapmessage(random("right","right","bottom right","top right")+" " + random("ass!","asscheek!","buttcheek!","cheek!","rear cheek","rear","sitspots!") ); 
	 if (fastorslow==0)
		{ playAudio("Audio/Spicy/Punishment/SpankingTask/30Strikes/Spank30.mp3", true);
		 sleep(50);
		}
		else
		{ MXoverlapmessage(random("Fast ones!","Quick ones!","Speedy ones!"));
		 playAudio("Audio/Spicy/Punishment/SpankingTask/30Strikes/Spank30Fast.mp3", true);
		sleep(25);
		}
		
	break;
	
	
		case 40:
	fastorslow=randomInteger(0,1);
	 setVar("SpankingCount",getVar("SpankingCount")+40 );
	 	MXoverlapmessage(random("left","left","bottom left","top left")+" " + random("ass!","asscheek!","buttcheek!","cheek!","rear cheek","rear","sitspots!") ); 
	 if (fastorslow==0)
		{ playAudio("Audio/Spicy/Punishment/SpankingTask/40Strikes/Spank40.mp3", true);
		 sleep(67);
		}
		else
		{ MXoverlapmessage(random("Fast ones!","Quick ones!","Speedy ones!"));
		 playAudio("Audio/Spicy/Punishment/SpankingTask/40Strikes/Spank40Fast.mp3", true);
		sleep(33);
		}
		
	fastorslow=randomInteger(0,1);
	 setVar("SpankingCount",getVar("SpankingCount")+40 );
	 	MXoverlapmessage(random("right","right","bottom right","top right")+" " + random("ass!","asscheek!","buttcheek!","cheek!","rear cheek","rear","sitspots!") ); 
	 if (fastorslow==0)
		{ playAudio("Audio/Spicy/Punishment/SpankingTask/40Strikes/Spank40.mp3", true);
		 sleep(67);
		}
		else
		{ MXoverlapmessage(random("Fast ones!","Quick ones!","Speedy ones!"));
		 playAudio("Audio/Spicy/Punishment/SpankingTask/40Strikes/Spank40Fast.mp3", true);
		sleep(33);
		}
		
	break;
	
	
		case 50:
	fastorslow=randomInteger(0,1);
	 setVar("SpankingCount",getVar("SpankingCount")+50 );
	 	MXoverlapmessage(random("left","left","bottom left","top left")+" " + random("ass!","asscheek!","buttcheek!","cheek!","rear cheek","rear","sitspots!") ); 
	 if (fastorslow==0)
		{ playAudio("Audio/Spicy/Punishment/SpankingTask/50Strikes/Spank50.mp3", true);
		 sleep(83);
		}
		else
		{ MXoverlapmessage(random("Fast ones!","Quick ones!","Speedy ones!"));
		 playAudio("Audio/Spicy/Punishment/SpankingTask/50Strikes/Spank50Fast.mp3", true);
		sleep(40);
		}
		
	fastorslow=randomInteger(0,1);
	 setVar("SpankingCount",getVar("SpankingCount")+50 );
	 	MXoverlapmessage(random("right","right","bottom right","top right")+" " + random("ass!","asscheek!","buttcheek!","cheek!","rear cheek","rear","sitspots!") ); 
	 if (fastorslow==0)
		{ playAudio("Audio/Spicy/Punishment/SpankingTask/50Strikes/Spank50.mp3", true);
		 sleep(83);
		}
		else
		{ MXoverlapmessage(random("Fast ones!","Quick ones!","Speedy ones!"));
		 playAudio("Audio/Spicy/Punishment/SpankingTask/50Strikes/Spank50Fast.mp3", true);
		sleep(40);
		}
		
	break;
	
	
	

}
}

//(End1)

guess= sendInput(" How many strokes were that slave? "); 
if (guess.getInt() == getVar("SpankingCount"))
	{MXmessage(" Correct! ");
	 setVar("TotalSpanking",getVar("TotalSpanking") + getVar("SpankingCount"));
	}
	else
	{
	MXmessage(" Wrong! ");
	MXmessage(" The number was "+getVar("SpankingCount") );
	 changeMeritMedium(true);
	 chance= random(1,5);
	 if (chance>2)
	 {
		MXmessage(" Since you've failed to count I guess we're starting over! "); 
		run("Dungeon/SpankzChoir/MXSpanking.js");
	 }
	 else
	 {
	
		MXmessage(" I'm gonna be merciful this time ");
		setVar("TotalSpanking",getVar("TotalSpanking") + getVar("SpankingCount"));
		MXmessage(" Don't expect me to show kindness again! ");
	 }

	}
	
	
	
	
